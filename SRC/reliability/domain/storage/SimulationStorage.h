/* ****************************************************************** **
**    OpenSees - Open System for Earthquake Engineering Simulation    **
**          Pacific Earthquake Engineering Research Center            **
**                                                                    **
**                                                                    **
** (C) Copyright 1999, The Regents of the University of California    **
** All Rights Reserved.                                               **
**                                                                    **
** Commercial use of this program without express permission of the   **
** University of California, Berkeley, is strictly prohibited.  See   **
** file 'COPYRIGHT'  in main directory for inSimulationation on usage and   **
** redistribution,  and for a DISCLAIMER OF ALL WARRANTIES.           **
**                                                                    **
** Developed by:                                                      **
**   Frank McKenna (fmckenna@ce.berkeley.edu)                         **
**   Gregory L. Fenves (fenves@ce.berkeley.edu)                       **
**   Filip C. Filippou (filippou@ce.berkeley.edu)                     **
**                                                                    **
** ****************************************************************** */
                                                        
                                                                        
// Written: Kevin Mackie 
//
// Description: This file contains the SimulationStorage class interface

#ifndef SimulationStorage_h
#define SimulationStorage_h

#include <ReliabilityStorage.h>
#include <Information.h>
#include <Vector.h>


class SimulationStorage : public ReliabilityStorage
{
 public:
    SimulationStorage();
    ~SimulationStorage();
  
    const char *getClassType(void) const;
    
    int setVariable(const char *variable, Information &);
    int getVariable(const char *variable, Information &);

private:
    double beta;
    double pf;
    double cov;
    double qbar;
    double rstdv;
    
};

#endif
